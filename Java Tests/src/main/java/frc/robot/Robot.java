...
public class Robot {
	...
	public static class DriveBase extends SubsystemBase {	// extend DifferentialDrive?
		...
		private Constants.DriveMode mode = Constants.DriveMode.TANK;	// make a constant for starting drive mode
		public boolean default_squaring = false;	// private w/ setter & getter?

		public DriveBase(Constants.DB2 map) {
			...
			setDefaultCommand(new Idling(this));
		}
		public DriveBase(Constants.DB4 map) {
			this.left = new SpeedControllerGroup(new PWMVictorSPX(map.front_left), new PWMVictorSPX(map.back_left));
			this.right = new SpeedControllerGroup(new PWMVictorSPX(map.front_right), new PWMVictorSPX(map.back_right));
		...
		public void incrementMode(int v) {
			this.mode = this.mode.increment(v);
		}
		public void decrementMode() {
			this.mode = this.mode.decrement();
		}
		public void decrementMode(int v) {
			this.mode = this.mode.decrement(v);
		}

		public void tankDrive(double lspeed, double rspeed) {
			this.drive.tankDrive(lspeed, rspeed, this.default_squaring);
		}
		public void arcadeDrive(double speed, double rotation) {
			this.drive.arcadeDrive(speed, rotation, this.default_squaring);
		}
		...
		public void raceDrive(double forward, double backward, double rotation) {
			this.drive.arcadeDrive(forward-backward, rotation, this.default_squaring);
		}
		...
		public void triggerDrive(double lspeed, double rspeed, boolean forward) {
			...
			this.drive.tankDrive(lspeed*mult, rspeed*mult, this.default_squaring);
		}
		public void modeDrive(Input controls) {
			switch(this.mode) {
				...
				// 	break;
				default:
					System.out.println("Drivemode detection error - motors not updated");
			}
		}
		...
		private static class Idling extends CommandBase {
			...
			@Override
			public void initialize() {System.out.println("Drive motors idling");}
			@Override
			public void execute() {drivebase.zero();}
			@Override 
			public void end(boolean i) {System.out.println("Drive motors no longer idling");}
			@Override
		...
		public static class Decelerate extends CommandBase {
			...
			public Decelerate(DriveBase db, Constants.Deceleration d) {		// << Add a constant for the gradient - this should be 0.96 to 0.99, but 0.98 should be best
				this.drivebase = db;
				this.s_left = this.db.getLeft();
				this.s_right = this.db.getRight();
			...
			@Override
			public void end(boolean i) {this.drivebase.zero();}
			@Override
			public boolean isFinished() {return (s_left < 0.1) && (s_right < 0.1);}
		}
	}

// Additioal robot parts can be added >>>
